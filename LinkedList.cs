...
namespace LinkedListOperations {
    public class LinkedList {
        ...
        public void DeleteNode (int n) {
            ...
            // one. Since we don’t have a pointer to the previous node, 
            // we can’t redirect its next pointer. So what do we do? 
            // We can easily get away by moving the data from the next node into the current node 
            // and then deleting the next node.
            var nodeToDelete = GetNode (n);
            var temp = nodeToDelete.Next.Next;
